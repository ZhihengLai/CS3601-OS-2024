...
0x00000000004008c8 <+0>:	stp	x29, x30, [sp, #-32]!
   ...
   0x00000000004008d0 <+8>:	add	x2, sp, #0x10           //x2 = sp + #0x10
=> 0x00000000004008d4 <+12>:	strb	wzr, [x2, w1, sxtw]        //将wzr（值为0的字节）存储到x2 + SignExtend(w1)地址指定的内存位置
   0x00000000004008d8 <+16>:	add	w3, w1, w1, lsr #31         //w1 = 3, w3 = w1 + 0 = 3
   0x00000000004008dc <+20>:	asr	w3, w3, #1              //w3 = 1
   0x00000000004008e0 <+24>:	cmp	w1, #0x1        //w1 > 1，下一步不跳转
   0x00000000004008e4 <+28>:	b.le	0x40095c <encrypt_method1+148>
   0x00000000004008e8 <+32>:	mov	x4, x2             //x4 = x2 = sp + #0x10
   0x00000000004008ec <+36>:	mov	x2, #0x0                   	// x2 = #0
   0x00000000004008f0 <+40>:	lsl	x5, x2, #1        //第一次：x5 = 0
   0x00000000004008f4 <+44>:	ldrb	w5, [x0, x5]       //w5 = (x0 + x5)[w5]
   0x00000000004008f8 <+48>:	strb	w5, [x4], #1    //[x4] = w5, x4++
   0x00000000004008fc <+52>:	add	x2, x2, #0x1            //x2++
   0x0000000000400900 <+56>:	cmp	w3, w2 // w3 = w2 = 1
   0x0000000000400904 <+60>:	b.gt	0x4008f0 <encrypt_method1+40>  //skipped
   0x0000000000400908 <+64>:	cmp	w3, #0x0     //w3 = 1 > 0
   0x000000000040090c <+68>:	csinc	w5, w3, wzr, gt     //w5 = w3 = 1
   0x0000000000400910 <+72>:	cmp	w1, w5 //w1 = 3 > 1
   0x0000000000400914 <+76>:	b.le	0x40094c <encrypt_method1+132> //第一次不跳转
   ...
   0x0000000000400928 <+96>:	mov	x1, #0x0                   	// x1 = #0
   0x000000000040092c <+100>:	add	x3, sp, #0x10             //x3 = sp + #0x10, 指针
   0x0000000000400930 <+104>:	add	x5, x3, w5, sxtw          //x5 = x3 + w5 = x3 + 1，指针右移一个字节
   0x0000000000400934 <+108>:	lsl	x3, x1, #1             //第一次x3 = 0, 第二次x3 = 2.
   0x0000000000400938 <+112>:	ldrb	w3, [x2, x3]              //从[x2, x3]加载一个字节到w3。
   0x000000000040093c <+116>:	strb	w3, [x5, x1]           //从w3加载一个字节到[x5, x1]
   0x0000000000400940 <+120>:	add	x1, x1, #0x1 //x1 ++
   0x0000000000400944 <+124>:	cmp	x1, x4 //x4 = 2，第一次x1 = 1，第二次相等
   0x0000000000400948 <+128>:	b.ne	0x400934 <encrypt_method1+108>  // b.any
   0x000000000040094c <+132>:	add	x1, sp, #0x10
   0x0000000000400950 <+136>:	bl	0x421cc0 <strcpy> // after this *x0's value is set to *x1，*x0被存在x1所指向的栈上的区域
   0x0000000000400954 <+140>:	ldp	x29, x30, [sp], #32   //sp += 32
...
```assembly
=> 0x0000000000400964 <+0>:	cmp	w1, #0x0         //w1 = 3
   0x0000000000400968 <+4>:	b.le	0x4009e0 <encrypt_method2+124>
   0x000000000040096c <+8>:	stp	x29, x30, [sp, #-48]!  //sp -= 48
   0x0000000000400970 <+12>:	mov	x29, sp
   0x0000000000400974 <+16>:	stp	x19, x20, [sp, #16] 
   0x0000000000400978 <+20>:	stp	x21, x22, [sp, #32]   //sp + 32 - 48 + 32 = sp + #0x10
   0x000000000040097c <+24>:	mov	x19, x0         //x19 = x0，input字符串指针
   0x0000000000400980 <+28>:	add	x21, x0, w1, sxtw    //x21 = x0 + 3，字符串第三位
   ...
   0x00000000004009b0 <+76>:	mov	x20, x19  // before this op, x20 = 3，此后x20保存x19指针2
   0x00000000004009b4 <+80>:	ldrb	w0, [x19]  // x19 is the string remained to be encoded, load x19[0] to w0
   0x00000000004009b8 <+84>:	sub	w0, w0, #0x61
   0x00000000004009bc <+88>:	and	w0, w0, #0xff // get the char's distance to 'a', like 'c' - 'a' = 2
   0x00000000004009c0 <+92>:	cmp	w0, #0x19         //确保初始w0在小写字母范围内
   0x00000000004009c4 <+96>:	b.ls	0x400990 <encrypt_method2+44>  // 回+44行
...
```assembly
   0x0000000000400a4c <+0>:	cbz	x1, 0x400ab8 <func_5+108> //[x1] = 49, not jump 
   0x0000000000400a50 <+4>:	stp	x29, x30, [sp, #-32]!
   0x0000000000400a54 <+8>:	mov	x29, sp
   0x0000000000400a58 <+12>:	stp	x19, x20, [sp, #16]
   0x0000000000400a5c <+16>:	mov	w20, w0 // w20 = w0 = input
   0x0000000000400a60 <+20>:	mov	x19, x1 //*x19 = *x1 = 49
   0x0000000000400a64 <+24>:	ldr	w0, [x1] //w0 = 49
   0x0000000000400a68 <+28>:	cmp	w0, w20
   0x0000000000400a6c <+32>:	b.eq	0x400a98 <func_5+76>  // skipped
   0x0000000000400a70 <+36>:	ldr	w0, [x19] // w0 = 49
   0x0000000000400a74 <+40>:	cmp	w0, w20  //w20 = ?
   0x0000000000400a78 <+44>:	b.le	0x400aa0 <func_5+84> //if 49 < w20, to +84
   0x0000000000400a7c <+48>:	ldr	x1, [x19, #8] //x1 = 4849800
   0x0000000000400a80 <+52>:	mov	w0, w20 //w0 = w20 = input?
   ...
   0x0000000000400aa0 <+84>:	ldr	x1, [x19, #16]    //读取[x19, #16]到x1
   0x0000000000400aa4 <+88>:	mov	w0, w20     //w0 = w20 = input
   0x0000000000400aa8 <+92>:	bl	0x400a4c <func_5>
   0x0000000000400aac <+96>:	lsl	w0, w0, #1   //若这时w0 = 1
   0x0000000000400ab0 <+100>:	add	w0, w0, #0x1 //w0 = 3
